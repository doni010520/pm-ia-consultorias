...
/**
 * Busca conversa pendente por telefone
 */
export async function getConversation(phone) {
  if (pool) {
    const result = await query(
      `SELECT * FROM whatsapp_conversations
       WHERE phone = $1 AND state = 'waiting_confirmation'
...
/**
 * Salva conversa pendente
 */
export async function saveConversation(phone, organizationId, state, context) {
  if (pool) {
    const result = await query(
      `INSERT INTO whatsapp_conversations (organization_id, phone, state, context)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [organizationId, phone, state, JSON.stringify(context)]
    );
    return result.rows[0];
  }
  ...
  if (supabase) {
    const { data, error } = await supabase
      .from('whatsapp_conversations')
      .insert({ organization_id: organizationId, phone, state, context })
      .select()
      .single();
 ...
 */
export async function updateConversation(id, state, context) {
  if (pool) {
    const result = await query(
      `UPDATE whatsapp_conversations
       ...
       WHERE id = $3
       RETURNING *`,
      [state, context ? JSON.stringify(context) : null, id]
    );
    return result.rows[0];
  }
...
/**
 * Remove/finaliza conversa pendente
 */

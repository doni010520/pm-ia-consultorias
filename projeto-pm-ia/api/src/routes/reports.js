 ...
 * Envia relatório para o cliente via WhatsApp
 */
router.post('/:id/send', async (req, res, next) => {
  try {
    ...
    if (!whatsapp_number) {
      return res.status(400).json({ error: { message: 'whatsapp_number é obrigatório' } });
    }
    ...
    const { sendTextMessage, sendReport } = await import('../services/whatsapp.js');

    // Enviar resumo via WhatsApp
    const summary = `📊 *${report.title}*\n\nPeríodo: ${new Date(report.period_start).toLocaleDateString('pt-BR')} a ${new Date(report.period_end).toLocaleDateString('pt-BR')}`;

    const sendResult = await sendTextMessage(whatsapp_number, summary);

    // Enviar conteúdo do relatório (truncado para limite do WhatsApp)
    if (report.content_markdown) {
      const content = report.content_markdown.length > 4000
        ? report.content_markdown.substring(0, 4000) + '\n\n_...relatório truncado. Acesse o sistema para versão completa._'
        : report.content_markdown;
      await sendTextMessage(whatsapp_number, content);
    }
    
    if (!sendResult.success) {
      return res.status(500).json({ error: { message: 'Erro ao enviar mensagem', details: sendResult.error } });
    }
    
    // Atualizar registro
    await query(`
      UPDATE reports 
      SET sent_to = array_append(COALESCE(sent_to, '{}'), $1),
          sent_at = NOW()
      WHERE id = $2
    `, [whatsapp_number, id]);
